...
- `signingPin`: atiende `/api/signing-pin`, valida el token de Firebase Authentication (cuenta firmante activa) y crea o cambia el PIN de firma de 6 dígitos.
- `signActa`: atiende `/api/sign-acta`, valida el token de Firebase Authentication (cuenta firmante activa), verifica el PIN de firma y registra la Firma NODO sobre el acta cerrada.

**Importante:** `manageSigner`, `signingPin` y `signActa` son necesarias para que funcionen el registro de cuentas firmantes (módulo Accesos) y el Portal de Firmas. Si `/api/manage-signer` responde HTTP 404 (página HTML en vez de JSON), es señal de que estas funciones aún no se han desplegado en este proyecto.
